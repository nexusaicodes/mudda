...
We provide pre-built Docker images that can be used to run Mudda on your own server.

If you don't need to change the source code, and just want the out-of-the-box Mudda experience, this can be a great way to get started.

You'll find the latest version of Mudda's Docker image at `ghcr.io/nexus-ai/mudda:main`.
...
The standard Mudda setup keeps all of its storage inside the path `/rails/storage`.
...
```sh
docker run --volume mudda:/rails/storage ghcr.io/nexus-ai/mudda:main
```

That will create a named volume (called `mudda`) and mount it into the correct path.
...
To configure your Mudda installation, you can use environment variables.
Mudda has several of them.
...
Various features inside Mudda rely on cryptography to work (such as secure links).
...
You can use any long random string for this, or you can have the Mudda codebase generate one for you by running:
...
If you want the Mudda container to handle its own SSL automatically, you just need to specify the domain name that you're running it on.
...
So if you were running on `mudda.example.com` you could enable SSL like this:

```sh
docker run --publish 80:80 --publish 443:443 --env TLS_DOMAIN=mudda.example.com ...
```

If you are terminating SSL in some other proxy in front of Mudda, then you don't need to set `TLS_DOMAIN`, and can just publish port 80:
```sh
...
Mudda needs to be able to send email for its sign up/sign in flow, and for its regular summary emails.
...
You can then plug all your SMTP settings from that provider into Mudda via the following environment variables:

- `MAILER_FROM_ADDRESS` - the "from" address that Mudda should use to send email
...
Mudda needs to know the public URL of your instance so it can generate correct links in certain situations (like when sending emails).
Set `BASE_URL` to the full URL where your Mudda instance is accessible:

```sh
docker run --env BASE_URL=https://mudda.example.com ...
```

#### VAPID keys

Mudda can also send Web Push notifications.
...
By default, when you run the Mudda Docker image you'll be limited to creating a single account (although that account can have as many users as you like).
...
Here's an example of a `docker-compose.yml` that you could use to run Mudda via `docker compose up`

```yaml
services:
  web:
    image: ghcr.io/nexus-ai/mudda:main
    ...
    environment:
      ...
      - TLS_DOMAIN=mudda.example.com
      - BASE_URL=https://mudda.example.com
      - MAILER_FROM_ADDRESS=mudda@example.com
