...
class Cards::NotesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "create" do
    assert_difference -> { cards(:logo).notes.count }, +1 do
      post board_card_notes_path(cards(:logo).board, cards(:logo)), params: { note: { body: "Agreed." } }, as: :turbo_stream
    end
  ...
  test "update another user's note" do
    assert_no_changes -> { notes(:logo_agreement_jz).reload.body.to_s } do
      put board_card_note_path(cards(:logo).board, cards(:logo), notes(:logo_agreement_jz)), params: { note: { body: "I've changed my mind" } }, as: :turbo_stream
    end
  ...
  test "index as JSON" do
    ...
    get board_card_notes_path(card.board, card), as: :json
  ...
  test "create as JSON" do
    ...
    assert_difference -> { card.notes.count }, +1 do
      post board_card_notes_path(card.board, card), params: { note: { body: "New note" } }, as: :json
    end

    assert_response :created
    assert_equal board_card_note_path(card.board, card, Note.last, format: :json), @response.headers["Location"]
  ...
  test "create as JSON with custom created_at" do
    ...
    assert_difference -> { card.notes.count }, +1 do
      post board_card_notes_path(card.board, card), params: { note: { body: "Backdated note", created_at: custom_time } }, as: :json
    end
  ...
  test "show as JSON" do
    ...
    get board_card_note_path(note.card.board, note.card, note), as: :json

    assert_response :success
    assert_equal note.id, @response.parsed_body["id"]
    assert_equal note.card.id, @response.parsed_body.dig("card", "id")
    assert_equal board_card_url(note.card.board, note.card), @response.parsed_body.dig("card", "url")
    assert_equal board_card_note_url(note.card.board, note.card, note), @response.parsed_body["url"]
  end
  ...
  test "create as JSON with flat params" do
    ...
    assert_difference -> { card.notes.count }, +1 do
      post board_card_notes_path(card.board, card), params: { body: "Flat note" }, as: :json
    end
  ...
  test "update as JSON with flat params" do
    ...
    put board_card_note_path(cards(:logo).board, cards(:logo), note), params: { body: "Flat update" }, as: :json
  ...
  test "update as JSON" do
    ...
    put board_card_note_path(cards(:logo).board, cards(:logo), note), params: { note: { body: "Updated note" } }, as: :json
  ...
  test "destroy as JSON" do
    ...
    delete board_card_note_path(cards(:logo).board, cards(:logo), note), as: :json
